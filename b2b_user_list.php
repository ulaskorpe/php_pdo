...
foreach ($b2b as $company) {
	...
	?>
<tr></td><td><?=$company['name']?> <?=$company['surname']?> </td><td><?=$company['co_name']?> , <?=$company['iu_company']?></td><td><?=$company['stand_id']?></td><td><?=$company['email']?> / <?=$company['iu_telephone']?></td></tr>
<tr><td colspan="4" style="background-color: #ffffcc">
	<table width="100%">
<tr style="background-color: #acacac;color: #fff"><td>Saat</td><td> GÖRÜŞMECİ </td></tr>

		<?php
		$j=0;
		
			foreach ($appointments as $app) {
$color = ($j%2 ==0) ? "#eaeaea":"#ffffff";
				?>
				<tr style="background-color: <?=$color?>"><td><?=$app['slot_time']?></td><td><?=$app['u_name']?> <?=$app['u_surname']?>, <?=$app['u_email']?>, <?=$app['u_phone']?>, <?=$app['u_company']?></td></tr>
			<?php $j++; }
		?>
